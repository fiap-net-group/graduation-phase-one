...
namespace PoliceDepartment.EvidenceManager.API.Features.V1.Controllers
{
    ...
    {
        private readonly ILogin<LoginViewModel, BaseResponse<AccessTokenModel>> _login;

        public AuthorizationController(ILogin<LoginViewModel, BaseResponse<AccessTokenModel>> login)
        {
            _login = login;
        }

        [HttpPost]
        [ProducesResponseType(StatusCodes.Status200OK, StatusCode = StatusCodes.Status200OK, Type = typeof(BaseResponse<AccessTokenModel>))]
        ...
        public async Task<IActionResult> Login(LoginViewModel login, CancellationToken cancellationToken)
        {
            ...
            return BadRequest(response.ResponseDetails);
        }
    }
}
